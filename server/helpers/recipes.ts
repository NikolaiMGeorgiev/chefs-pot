...
import type { SelectedRecipeTypes } from "../types/recipes.js";
...
export function getUserRecipeSummaryQuery(selectedRecipeTypes: SelectedRecipeTypes) {
    const queryList = [];
...
export function getUserRecipesSummaryQueryParams(userId: number, selectedRecipeTypes: SelectedRecipeTypes) {
    const queryValues = [];
...
export function parseJsonFields(data: RowDataPacket[], fields: string[]) {
...
export function addRecipeImagePlaceholder(data: RowDataPacket[]) {
    for (let single of data) {
        if (!single.image) {
